...
public class PlayerStats : MonoBehaviour
{
    ...
    private Vector3 _originalMaxTextPos; // 원본 위치 저장용
    ...
    public bool RequestHarvestPermission(ResourcePlatform platform)
    {
        ...
        if (_currentHarvestTarget != null && _currentHarvestTarget != platform) return false;
    ...
    {
        if (_currentHarvestTarget == platform) _currentHarvestTarget = null;
    }
    ...
    public void AddResource(string resourceName, int amount)
    {
        ...
        if (visualStack != null)
        {
            for (int i = 0; i < canAdd; i++) visualStack.Add(resourceName);
        }
    }
    ...
    public void SpendResource(string resourceName, int amount)
    {
        if (_inventory.ContainsKey(resourceName))
        {
            ...
            if (visualStack != null)
            {
                for (int i = 0; i < amount; i++) visualStack.Remove(resourceName);
            }
        }
    }
    ...
    private IEnumerator FadeOutTextRoutine()
    {
        ...
        fullCapacityText.gameObject.SetActive(false);
        fullCapacityText.transform.localPosition = _originalMaxTextPos; // 위치 복구
    }
